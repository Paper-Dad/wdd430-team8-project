...
async function getProductDetails(id: string) {
  ...
  const product: ProductDetails = {
    ...
    images: Array.isArray(productDocument.images) ? productDocument.images : [],
    inventory: productDocument.inventory ?? 0,
    materials: Array.isArray(productDocument.materials)
      ? productDocument.materials
      : [],
    categoryName: productDocument.category?.name ?? "Uncategorized",
    categoryDescription: productDocument.category?.description ?? "",
    artisanId: productDocument.artisan?._id
      ? productDocument.artisan._id.toString()
      : null,
    artisanName:
      [productDocument.artisan?.firstName, productDocument.artisan?.lastName]
        .filter(Boolean)
        .join(" ") || "Unknown artisan",
    shopName: productDocument.artisan?.shopName ?? "Independent Artisan",
...
export default async function ProductDetailsPage({ params }: ProductPageProps) {
  ...
  const mainImage = product.images[0] ?? "/images/product-placeholder.jpg";
      ...
      <main className="product-details-page">
        ...
        <section className="product-details-layout">
          ...
          <div className="product-details-content">
            ...
            <p className={product.inventory > 0 ? "in-stock" : "out-of-stock"}>
              {product.inventory > 0
        ...
        <section className="product-artisan-section">
          ...
          <p className="product-artisan-name">{product.artisanName}</p>

          {product.artisanBio && <p>{product.artisanBio}</p>}

          {product.artisanId && (
            <Link href={`/artisans/${product.artisanId}`} className="button">
              View Artisan
            </Link>
          )}
        </section>
        ...
        >
          <div className="section-heading">
            <div>
              ...
              <h2 id="product-reviews-heading">Product Reviews</h2>
            </div>
